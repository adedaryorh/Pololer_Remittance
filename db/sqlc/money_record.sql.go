...
) VALUES ($1, $2, $3, $4) RETURNING id, customer_id, reference, status, amount
`
...
SELECT id, customer_id, reference, status, amount FROM money_records WHERE reference = $1
`
...
SELECT id, customer_id, reference, status, amount FROM money_records WHERE status = $1
`
